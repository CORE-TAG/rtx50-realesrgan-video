...
PLUGIN_NAME = "SrvggFused2TcPlugin"
PLUGIN_VERSION = "1"
...
def load_fused2_weights(weight_dir, first_layer):
    w1 = load_weights(weight_dir, first_layer)
    w2 = load_weights(weight_dir, first_layer + 1)
    return {
        "weight1": w1["weight"],
        "bias1": w1["bias"],
        "prelu1": w1["prelu"],
...
def build_engine(plugin_path, weight_dir, layer, n, h, w, io_format):
    logger = Logger()
    weights = load_fused2_weights(weight_dir, layer)
    plugin = create_plugin(plugin_path, weights)
    if plugin is None:
    ...
    layer_obj = network.add_plugin_v2([input_tensor], plugin)
    output_tensor = layer_obj.get_output(0)
...
def main():
    ...
    parser.add_argument("--plugin", default=os.path.join(os.path.dirname(__file__), "srvgg_fused2_tc_plugin.dll"))
    parser.add_argument(
        "--weights",
    ...
    print(
        f"plugin=srvgg_fused2_tc layers={args.layer:02d}-{args.layer + 1:02d} n={args.n} "
