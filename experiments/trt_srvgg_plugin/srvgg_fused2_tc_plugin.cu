...
constexpr char const* kPluginName = "SrvggFused2TcPlugin";
...
class SrvggFused2TcPlugin final : public IPluginV2DynamicExt
{
public:
    SrvggFused2TcPlugin() = default;

    SrvggFused2TcPlugin(
        std::vector<std::uint16_t> weight1,
        std::vector<std::uint16_t> bias1,
        std::vector<std::uint16_t> prelu1,
        std::vector<std::uint16_t> weight2,
        std::vector<std::uint16_t> bias2,
        std::vector<std::uint16_t> prelu2)
        : mWeight1(std::move(weight1))
        , mBias1(std::move(bias1))
        , mPrelu1(std::move(prelu1))
        , mWeight2(std::move(weight2))
        , mBias2(std::move(bias2))
        , mPrelu2(std::move(prelu2))
    {
    }

    SrvggFused2TcPlugin(void const* data, size_t)
    {
        char const* src = static_cast<char const*>(data);
        mWeight1 = read_vector(src);
        mBias1 = read_vector(src);
        mPrelu1 = read_vector(src);
        mWeight2 = read_vector(src);
        mBias2 = read_vector(src);
        mPrelu2 = read_vector(src);
    }
    ...
    {
        return serialized_vector_size(mWeight1) + serialized_vector_size(mBias1) + serialized_vector_size(mPrelu1)
            + serialized_vector_size(mWeight2) + serialized_vector_size(mBias2) + serialized_vector_size(mPrelu2);
    }

    void serialize(void* buffer) const noexcept override
    {
        char* dst = static_cast<char*>(buffer);
        write_vector(dst, mWeight1);
        write_vector(dst, mBias1);
        write_vector(dst, mPrelu1);
        write_vector(dst, mWeight2);
        write_vector(dst, mBias2);
        write_vector(dst, mPrelu2);
    }
    ...
    {
        auto* plugin = new SrvggFused2TcPlugin(mWeight1, mBias1, mPrelu1, mWeight2, mBias2, mPrelu2);
    ...
    size_t getWorkspaceSize(PluginTensorDesc const* inputDesc, int32_t, PluginTensorDesc const*, int32_t) const noexcept override
    {
        if (inputDesc == nullptr || inputDesc[0].dims.nbDims != 4)
        {
            return 0;
        }
        std::int64_t elems = static_cast<std::int64_t>(inputDesc[0].dims.d[0]) * kChannels * inputDesc[0].dims.d[2]
            * inputDesc[0].dims.d[3];
        return elems > 0 ? static_cast<size_t>(elems) * sizeof(half) : 0;
    }
        ...
        void* const* outputs, void* workspace, cudaStream_t stream) noexcept override
    {
        ...
        if (inputDesc[0].format == TensorFormat::kHWC8)
        {
            conv3x3_prelu_wmma_kernel<true><<<grid, block, shared, stream>>>(
                static_cast<half const*>(inputs[0]), static_cast<half const*>(mWeight1Dev),
                static_cast<half const*>(mBias1Dev), static_cast<half const*>(mPrelu1Dev),
                static_cast<half*>(workspace), n, h, w);
            conv3x3_prelu_wmma_kernel<true><<<grid, block, shared, stream>>>(
                static_cast<half const*>(workspace), static_cast<half const*>(mWeight2Dev),
                static_cast<half const*>(mBias2Dev), static_cast<half const*>(mPrelu2Dev),
...
private:
    ...
    bool upload_all()
    {
        bool ok = mWeight1.size() == kWeightCount && mBias1.size() == kVectorCount && mPrelu1.size() == kVectorCount
            && mWeight2.size() == kWeightCount && mBias2.size() == kVectorCount && mPrelu2.size() == kVectorCount
            && upload_one(mWeight1, &mWeight1Dev) && upload_one(mBias1, &mBias1Dev) && upload_one(mPrelu1, &mPrelu1Dev)
            && upload_one(mWeight2, &mWeight2Dev) && upload_one(mBias2, &mBias2Dev) && upload_one(mPrelu2, &mPrelu2Dev);
        mDeviceReady = ok;
        return ok;
    }
    ...
    void free_device()
    {
        cudaFree(mWeight1Dev);
        cudaFree(mBias1Dev);
        cudaFree(mPrelu1Dev);
        cudaFree(mWeight2Dev);
        cudaFree(mBias2Dev);
        cudaFree(mPrelu2Dev);
        mWeight1Dev = nullptr;
        mBias1Dev = nullptr;
    ...
    std::vector<std::uint16_t> mWeight1;
    std::vector<std::uint16_t> mBias1;
    std::vector<std::uint16_t> mPrelu1;
    std::vector<std::uint16_t> mWeight2;
    std::vector<std::uint16_t> mBias2;
    std::vector<std::uint16_t> mPrelu2;
    void* mWeight1Dev{nullptr};
    void* mBias1Dev{nullptr};
    void* mPrelu1Dev{nullptr};
    void* mWeight2Dev{nullptr};
    void* mBias2Dev{nullptr};
    void* mPrelu2Dev{nullptr};
    bool mDeviceReady{false};
    std::string mNamespace;
};

class SrvggFused2TcCreator final : public IPluginCreator
{
public:
    SrvggFused2TcCreator()
    {
        mFields.emplace_back("weight1", nullptr, PluginFieldType::kFLOAT16, kWeightCount);
        mFields.emplace_back("bias1", nullptr, PluginFieldType::kFLOAT16, kVectorCount);
        mFields.emplace_back("prelu1", nullptr, PluginFieldType::kFLOAT16, kVectorCount);
        mFields.emplace_back("weight2", nullptr, PluginFieldType::kFLOAT16, kWeightCount);
        mFields.emplace_back("bias2", nullptr, PluginFieldType::kFLOAT16, kVectorCount);
        mFields.emplace_back("prelu2", nullptr, PluginFieldType::kFLOAT16, kVectorCount);
    ...
    IPluginV2* createPlugin(AsciiChar const*, PluginFieldCollection const* fc) noexcept override
    {
        ...
        for (int i = 0; i < fc->nbFields; ++i)
        {
            ...
            copy_field(field, "weight1", kWeightCount, weight1);
            copy_field(field, "bias1", kVectorCount, bias1);
            copy_field(field, "prelu1", kVectorCount, prelu1);
            copy_field(field, "weight2", kWeightCount, weight2);
            copy_field(field, "bias2", kVectorCount, bias2);
            copy_field(field, "prelu2", kVectorCount, prelu2);
        }
        if (weight1.size() != kWeightCount || bias1.size() != kVectorCount || prelu1.size() != kVectorCount
            || weight2.size() != kWeightCount || bias2.size() != kVectorCount || prelu2.size() != kVectorCount)
        {
            return nullptr;
        }
        auto* plugin = new SrvggFused2TcPlugin(std::move(weight1), std::move(bias1), std::move(prelu1),
            std::move(weight2), std::move(bias2), std::move(prelu2));
        plugin->setPluginNamespace(mNamespace.c_str());
        return plugin;
    }
    ...
    {
        auto* plugin = new SrvggFused2TcPlugin(serialData, serialLength);
        plugin->setPluginNamespace(mNamespace.c_str());
        return plugin;
    }
