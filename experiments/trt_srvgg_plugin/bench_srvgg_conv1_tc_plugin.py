...
def build_engine(plugin_path, weight_dir, layer, n, h, w, io_format):
    logger = Logger()
    weights = load_weights(weight_dir, layer)
...
def check_correctness(engine, weight_dir, layer, n, h, w, io_format):
    ...
    y_ref = reference_conv_prelu(x_nchw, load_weights(weight_dir, layer)).astype(np.float16).astype(np.float32)
...
def main():
    ...
    print(
        f"plugin=srvgg_conv1_tc layer={args.layer:02d} n={args.n} "
        f"shape={args.h}x{args.w} format={args.format} iters={args.iters}"
    )
    print(f"one_layer_ms={per_ms:.6f} layer_fps={fps:.3f} effective_tflops={tflops:.3f}")
    if args.check:
